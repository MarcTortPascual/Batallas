...
Boton::Boton(int x,int y,int gapX,int gapY, SDL_Color* color, SDL_Color* colortext,std::string text,SDL_Event* e)
{
    ...
    void Boton::check_status(){
        ...
        while (!this->stop) {
            el.lock();
            if (this->el.e.type == SDL_MOUSEMOTION){
               
                std::cout << el.e.motion.x << std::endl;
                this->hover = (this->el.e.motion.x > this->x && x < this->x + (this->w + this->gapX)) && (y > this->y && y < this->y + (this->h + this->gapY));
